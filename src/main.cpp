...
void vTaskSensors(void *pvParameters)
{
	...
	uint16_t error = NoError;
	char errorMessage[256];
	dataOffline_t data;
	bool data_ready = false, is_server_up = false;
	...
	for (;;)
	{
		...
		scd40.getDataReadyFlag(data_ready);
		if (data_ready == true and xSemaphoreTake(xI2C, portMAX_DELAY) == pdPASS)
		{
		...
		else if (debug_msk & (1 << debug_sensors))
		{
			...
			id_n_talk(pcTaskName, "SCD bad measurement: " + (String)errorMessage);
		}
...
void vTaskOffline(void *pvParameters)
{
	...
	bool conf_active, validated_state = false;
...
void vDaemon(void *pvParameters)
{
	...
	for (;;)
	{
		...
		if (n >= 10)
		{
			...
			{
				strftime(formated_time, 80, "Last known time: %I:%M%p.", &timeinfo);
			}
			n = 0;
		}
...
void vTaskWManager(void *pvParameters)
{
	...
	if (!res)
	{
		...
		{
			clear_lcd();
			scroll_text("         Con to AP Failed. Restarting now", LCD_SPD, 0);
			xSemaphoreGive(xLCDAccess);
		}
		// reconnect
...
void setup()
{
	...
	// WEB SERVER
	server_init_stuff(on_now, on_fcast, data_offline, data_online, CLOCK_PENDING);
