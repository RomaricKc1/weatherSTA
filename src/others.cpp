...
// RTC RAM DATA
RTC_DATA_ATTR float SEALEVELPRESSURE_HPA_2 = 1020; // this will be updated when esp goes online
...
// LCD
byte degre[8] = {B00110, B01001, B01001, B00110, B00000, B00000, B00000, B00000};
...
void fcastLogic(const char *pcTaskName, const String serverPath_forecast, dataOnline_t &data_f)
{
    HTTPClient http;
    WiFiClient client;
    ...
    if (error)
    {
        if (debug_msk & (1 << debug_api))
        {
            id_n_talk(pcTaskName, "Forecast Error code:deserializeJson() failed: " + (String)error.c_str());
        }
        return;
    }
...
void nowLogic(const char *pcTaskName, const String serverPath_now, dataOnline_t &data_n, float &last_press)
{
    HTTPClient http;
    WiFiClient client;
    ...
    if (error)
    {
        if (debug_msk & (1 << debug_api))
        {
            id_n_talk(pcTaskName, "Now Error code:deserializeJson() failed: " + (String)error.c_str());
        }
        return;
    }
    ...
    (SEALEVELPRESSURE_HPA_2 == 0) ? SEALEVELPRESSURE_HPA_2 = last_press : last_press = SEALEVELPRESSURE_HPA_2;
...
{
    // mutexes and queues (they are static var, no tied to the function whatsoever)
                     ...
                     bool &clk_mode, bool &valid_clk, bool &ghost_timer, bool &tmr_flag)
{
    ...
    case 2: // dec menu (x3 click)
        ...
        {
            menu_n.current_menu = MENUS - 1; // allow to cycle the menu e.g. current menu: 5, 4, 3
        }
        else
        {
    ...
    case 1: // inc menu (2x click)
        ...
        {
            menu_n.current_menu = 0; // allow to cycle the menu e.g. current menu: 5, 1, 2
        }
        else
        {
            menu_n.current_menu++;
    ...
    case 0: // exec aka cycle the current menu page variable (1x click)
        ...
        // pick the value for this exact index and menu page using preset available, and inc the idx
    ...
    case 3: // exit menu before timeout (long press)
        // checks the different state, validate the bit, remove the current timer, and create a new one really fast (like 2ms) that will expire right away
...
void applyMenuSave(menu_t &menu_n, long &current_param, bool &clk_24_mode, bool &chg_wlan, bool &clk_mode)
{
    ...
    case 2: // sleep param
        if (current_param != 0)
        {
            ...
            xQueueOverwrite(xIdle_T, (void *)&IDLE_TMS); // local so gotta send it to the queue
        }
        break;
...
void exitBeforeTimeout(bool &clk_mode, bool &valid_clk, bool &ghost_timer, bool &tmr_flag)
{
    EventBits_t event_result;
    /*event await timer_flag and valid_clk*/
    if (clk_mode == 1)
    {
        xEventGroupSetBits(handler_event, ((1 << V_CLK_STATE_BIT_SET) | (1 << V_CLK_STATE_BIT_LED))); // validate the mode now
        xEventGroupClearBits(handler_event, ((1 << WAIT_CLK_BIT_LED)));
    }
    event_result = xEventGroupWaitBits(handler_event, (1 << TIMER_FLAG_BIT), pdFALSE, pdFALSE, 0);
    ...
    if (tmr_flag == true)
    {
        ...
        bool res_timer = createTimer(pdMS_TO_TICKS(2), releaser_daemon, "vSetup", "one-x-timer");
...
void getClockData(time_var_t &time_vars)
{
    ...
    strftime(time_vars.timehh, sizeof(time_vars.timehh), "%I", &timeinfoLocal); // hour 12h
    strftime(time_vars.timehh_24, sizeof(time_vars.timehh_24), "%H", &timeinfoLocal);
    strftime(time_vars.timemm, sizeof(time_vars.timemm), "%M", &timeinfoLocal);     // minutes
    strftime(time_vars.timedd, sizeof(time_vars.timedd), "%d", &timeinfoLocal);     // day of the month
    strftime(time_vars.timemomo, sizeof(time_vars.timemomo), "%B", &timeinfoLocal); // month in text
    ...
    if (time_vars.timemm_int == 0 && (time_vars.timehh_24_int < 2 || time_vars.timehh_24_int > 6))
    {
                    ...
                    int &ticktime_date, int &ticktime_co2, int &ticktime_server, bool &db_server_state, const char *pcTaskName)
{
    ...
    if ((int)(round(my_data_offline.temp)) >= 0 && (int)(round(my_data_offline.temp)) < 10)
    {
        lcd.setCursor(1, 0);
        lcd.write(byte(0));
        lcd.setCursor(2, 0);
    ...
    else if (((int)(round(my_data_offline.temp)) < 0 && (int)(round(my_data_offline.temp)) > -10) || ((int)(round(my_data_offline.temp)) >= 10))
    {
        lcd.setCursor(2, 0);
        lcd.write(byte(0));
        lcd.setCursor(3, 0);
...
void printOutdoorLCD(const float outTemp)
{
    ...
    if (((int)(round(outTemp))) >= 0 and ((int)(round(outTemp)) < 10))
    { // 0, .. 9
        lcd.setCursor(13, 1);
        lcd.write(byte(0));
        lcd.setCursor(14, 1);
    ...
    else if (((int)(round(outTemp)) < 0 && (int)(round(outTemp)) > -10) || ((int)(round(outTemp)) >= 10))
    { //-9, ..., -1   or   10, 11, 12, ...
        lcd.setCursor(14, 1);
        lcd.write(byte(0));
        lcd.setCursor(15, 1);
...
void onlineTaskPrint(const bool conf_active, dataOnline_t &my_data, bool &validated_state_now, bool &validated_state_fcast, bool &read_now, String &_id)
{
    BaseType_t rc = pdFALSE;
    ...
    (read_now) ? rc = xQueuePeek(xQueueOn_NOW, (void *)&my_data, 0) : rc = xQueuePeek(xQueueOn_FCAST, (void *)&my_data, 0);
    ...
    if (rc == pdPASS and (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS))
    {
        ...
        {
            // now
            bool now_int = scroll_text("                 weather NOW    ", LCD_SPD, 0);
            (now_int == true) ? validated_state_now = false : validated_state_now = true;
        ...
        {
            // fcast
            bool fcast_int = scroll_text("                FORECAST weather", LCD_SPD, 0);
            (fcast_int == true) ? validated_state_fcast = false : validated_state_fcast = true;
        ...
        if (xSemaphoreTake(xI2C, portMAX_DELAY) == pdPASS)
        {
            if (((int)(round(my_data.temp))) < -9 || ((int)(round(my_data.tempMin))) < -9 || ((int)(round(my_data.tempMax))) < -9)
            {
            ...
            { // general stuff from -9 to 99 °, above 99 lol I don't care, below -99, I don't care either lol
                lcd.setCursor(0, 0);
                lcd.print("TEMP:");
                lcd.setCursor(5, 0);
                lcd.print((int)(round(my_data.temp)));
        ...
        // others
        String text2 = "                Feels like: " + (String)my_data.feels_like + (String)((char)223) + ", Humidity: " + (String)((int)my_data.humidity) + "%, Pressure: " + (String)((int)my_data.pressure) + "hPa, Wind Speed: " + (String)my_data.windSpeed + "m/s, Cloud coverage: " + (String)((int)my_data.clouds) + "% " + (String)(my_data.description);
...
void offlineTaskPrint(const bool conf_active, dataOffline_t &my_data, bool &validated_state)
{
    BaseType_t rc = pdFALSE;
    ...
    if (rc == pdPASS and (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS))
    {
        ...
        bool interrupted_ = scroll_text("                Indoors weather", LCD_SPD, 0);
        ...
        String text2 = "                Humidity: " + (String)my_data.humi + "%, Co2: " + co2_text + "ppm, Altitude: " + (String)my_data.altitude + "m, Pressure: " + (String)my_data.pressure + "hPa";
...
bool scdSetup()
{
    ...
    if (error)
    {
        if (debug_msk & (1 << debug_sensors))
        {
            ...
            id_n_talk(pcTaskName, "Can't stopPeriodicMeasurement(): " + (String)errorMessage);
        }
    ...
    if (error)
    {
        ...
        {
            errorToString(error, errorMessage, 256);
            id_n_talk(pcTaskName, "Unable to startPeriodicMeasurement(): " + (String)errorMessage);
        }
    ...
    scd_cal_one_x_timer = xTimerCreate("scd_calibration_timer", pdMS_TO_TICKS(SCD_CAL_TMS), pdFALSE, (void *)0, scd_cal_daemon);
    xEventGroupClearBits(handler_event, (1 << SCD_CAL_DONE)); // set the calibration bit to 0
...
bool scroll_text(const String text, const int delay, const int line)
{
    ...
    event_result = xEventGroupWaitBits(handler_event, ((1 << CONF_ACTIVE_BIT_SCR)), pdFALSE, pdFALSE, 0);
    (event_result & (1 << CONF_ACTIVE_BIT_SCR)) ? conf_active = true : conf_active = false;
...
void server_init_stuff(on_Data_t &on_now, on_Data_t &on_fcast, dataOffline_t &data_offline, dataOnline_t &data_online, int &STATE)
{
    ...
    // 				O							U							T
    server.on("/outtemperature_now", HTTP_GET, [&](AsyncWebServerRequest *request)
    ...
    server.on("/outdescription_now", HTTP_GET, [&](AsyncWebServerRequest *request)
    ...
    server.on("/outtemperature_fcast", HTTP_GET, [&](AsyncWebServerRequest *request)
              { request->send(200, "text/plain", (String)on_fcast.temp); });
    server.on("/outhumidity_fcast", HTTP_GET, [&](AsyncWebServerRequest *request)
              { request->send(200, "text/plain", (String)on_fcast.humidity); });
    server.on("/outpressure_fcast", HTTP_GET, [&](AsyncWebServerRequest *request)
    ...
    server.on("/outfeelslike_fcast", HTTP_GET, [&](AsyncWebServerRequest *request)
              { request->send(200, "text/plain", (String)on_fcast.feels_like); });
    server.on("/outdescription_fcast", HTTP_GET, [&](AsyncWebServerRequest *request)
              { request->send(200, "text/plain", (String)on_fcast.description); });
    server.on("/outwindspeed_fcast", HTTP_GET, [&](AsyncWebServerRequest *request)
...
void getServer_State(bool &state, const char *pcTaskName)
{
    ...
    if (debug_msk & (1 << debug_dbserver))
    {
        if (state)
        {
            id_n_talk(pcTaskName, "Connected to InfluxDB: " + db_client.getServerUrl());
        }
        else
        {
            id_n_talk(pcTaskName, "InfluxDB connection failed: " + db_client.getLastErrorMessage());
        }
    }
}
...
bool writePointToServer(Point &sensor, dataOffline_t data, const char *pcTaskName, int &failed_try)
{
    ...
    if (db_client.writePoint(sensor) == false)
    {
        if (debug_msk & (1 << debug_dbserver))
        {
            id_n_talk(pcTaskName, "InfluxDB write failed: " + db_client.getLastErrorMessage());
        }
        failed_try += 1;
        return false;
    }
...
void readSysConfig()
{
    // now, try to read the confi_file (api_key and location) to use it. If there's no data well, launch the wifi Portal
    if (!(SPIFFS.exists(CONFIG_FILE)))
    {
        ...
        if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS)
        {
            ...
            scroll_text("        will timeout after 6mins, starting now.", LCD_SPD, 1);
            xSemaphoreGive(xLCDAccess);
        }
        ...
        if (!res)
        {
            ...
            // then what next? restart? yes, after the timeout so it's long, but user can hit restart right away
        }
        else
        {
            clear_lcd();
...
bool createTimer(const long duration, TimerCallbackFunction_t(call_func_ptr), const char *pcTaskName, const char *timerName)
{
    ...
    one_x_timer_releaser = xTimerCreate(timerName, pdMS_TO_TICKS(duration), pdFALSE, (void *)0, call_func_ptr);
                   ...
                   const int config_T, const char *pcTaskName, int &config_msk, TickType_t &xLastWakeTime_c)
{
    ...
    vTaskSuspend(xTaskOnline); // suspend those who have access to the lcd mainly (the led with the clock is already cleared, we don't want to stop this task)
    ...
    handleMenuLogic(menu_n, i, data_nexter, current_param, clk_24_mode, chg_wlan, clk_mode, valid_clk, ghost_timer, tmr_flag);
    STATES_T.flag[i] = 0;
    ...
    // create the timer for the timeout
    EventBits_t event_result = xEventGroupWaitBits(handler_event, ((1 << TIMER_FLAG_BIT)), pdFALSE, pdFALSE, 0); /*event await timer_flag*/
    (event_result & (1 << TIMER_FLAG_BIT)) ? tmr_flag = true : tmr_flag = false;

    launchMenuTimout(tmr_flag, ghost_timer, config_T, pcTaskName, config_msk, xLastWakeTime_c);
}
...
void launchMenuTimout(const bool tmr_flag, const bool ghost_timer, const int config_T, const char *pcTaskName, int &config_msk, TickType_t &xLastWakeTime_c)
{
    ...
    // enter this to create the timer only once, not multiple times # countdown timer
    if (createTimer(config_T * 1000, releaser_daemon, pcTaskName, "one-x-timer") == pdPASS)
    {
        // prevent from launching multiple timers by settin it to 1, so next time the condition check will fail
...
void broadcastConfMode(bool clear /*  = false */)
{
    if (clear == false)
    {
        ...
        xEventGroupSetBits(handler_event, ((1 << CONF_ACTIVE_BIT_ON) | (1 << CONF_ACTIVE_BIT_LED) | (1 << CONF_ACTIVE_BIT_SCR) | (1 << CONF_ACTIVE_BIT_DAE) | (1 << CONF_ACTIVE_BIT_OFF)));
    }
    else
    {
        // remove the config mode
        xEventGroupClearBits(handler_event, ((1 << CONF_ACTIVE_BIT_ON) | (1 << CONF_ACTIVE_BIT_LED) | (1 << CONF_ACTIVE_BIT_SCR) | (1 << CONF_ACTIVE_BIT_DAE) | (1 << CONF_ACTIVE_BIT_OFF)));
    }
}
...
void clockOrStaMode()
{
    ...
    case 1:
    {
        ...
        if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS)
        {
            ...
            xEventGroupSetBits(handler_event, ((1 << V_CLK_STATE_BIT_SET) | (1 << V_CLK_STATE_BIT_LED))); // validate clock mode now

            xSemaphoreGive(xLCDAccess);
        }

        // to prevent this from running all the way everytime, putting -1 won't allow the if execution again
...
void checkCExecPortal(const bool valid_portal)
{
    ...
    if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS)
    {
        ...
        xEventGroupClearBits(handler_event, ((1 << WLAN_CONTD_BIT))); // stop sending http requests
                  ...
                  TickType_t &xtick_unknown, int &cnt, TickType_t &xLastLedOn, bool &flicker_led, bool &last_flick_state)
{
    ...
    if (((xTaskGetTickCount() - xTick_wlan_state) >= 1 * 60 * 1000) and (WiFi.status() != WL_CONNECTED))
    {
    ...
    else
    {
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            xEventGroupClearBits(handler_event, ((1 << LED_BUSY))); // also clear this when coed
        }
    }
...
void pirLogic(bool &pir_state, bool &last_pir, TickType_t &last_detected_pir, const bool valid_clk)
{
    ...
    if ((pir_state != last_pir) && (pir_state == true) && (xSemaphoreTake(xLCDAccess, 200) == pdPASS))
    {
        // this helps counter the fact of writing true true or false false repeatedly
        // also, only write true, the false will be taken care
    ...
    if (((xTaskGetTickCount() - last_detected_pir) > PIR_COUNTER_DELAY) and xSemaphoreTake(xLCDAccess, 200) == pdPASS) // added 1 s to it
    {
        if (xSemaphoreTake(xI2C, 200) == pdPASS)
        {
            lcd.noBacklight();
...
void idleSTAMode(bool conf_active, bool &hard_resume, int &rep, bool &check_go_idle_now)
{
    ...
    {
        // config is active
        hard_resume = true; // give it a chance to comeback again and go to idle mode and then unclock the whole sys again
        // since the SYS_STATE_IDLE is active, offline and online shutdown themselves lol waiting for the savor to occurs lol.
        // xSemaphoreGive(xLCDAccess); // give it to config thing, and it will block you lol
        return;
    }
    ...
    // if the thing is not active, then go ahead, otherwise, leave what you are doing and give the mutex.
    // also turn off the lcd disp
    if (xSemaphoreTake(xI2C, portMAX_DELAY) == pdPASS)
    {
        vTaskDelay(pdMS_TO_TICKS(1000));
    ...
    else
    {
        ...
        rep += 1; // and yea, only if you did yo thing nicely you can count it, not while config was waiting
...
void taskNotifMan(uint32_t rv, uint32_t &rv_msk, bool &idle)
{
    ...
    if (rv_msk == ((1 << SYSNOTIF_IN) | (1 << SYSNOTIF_FCAST) | (1 << SYSNOTIF_NOW)))
    {
        idle = true;                                                                   // all cases met
        rv_msk &= ~((1 << SYSNOTIF_IN) | (1 << SYSNOTIF_FCAST) | (1 << SYSNOTIF_NOW)); // clear the state flag
    }
    else
    {
        idle = false;
...
void saveParamCallback()
{
    ...
    if (!config)
    {
        ...
        {
            clear_lcd();
            scroll_text("                Error opening config file. Rebooting ...", LCD_SPD, 0);
...
void releaser_daemon(TimerHandle_t xTimer)
{
    ...
    xEventGroupClearBits(handler_event, ((1 << WAIT_CLK_BIT_LED) | (1 << TIMER_FLAG_BIT) | (1 << BTN_WAIT_MODE_BIT)));
    ...
    {
        xEventGroupClearBits(handler_event, ((1 << V_CLK_STATE_BIT_SET) | (1 << V_CLK_STATE_BIT_LED) | (1 << CONF_ACTIVE_BIT_ON) | (1 << CONF_ACTIVE_BIT_OFF)));
        xEventGroupClearBits(handler_event, ((1 << CONF_ACTIVE_BIT_LED) | (1 << CONF_ACTIVE_BIT_SCR) | (1 << CONF_ACTIVE_BIT_DAE) | (1 << CONF_ACTIVE_BIT_SET)));
    ...
    if (chg_config == true)
    {
        xEventGroupSetBits(handler_event, ((1 << V_START_PORTAL_BIT_SET))); // validate the portal mode now
                                                                            // stop others ta , cause, we won't need them until next reboot
    }
    else
    {
        xEventGroupClearBits(handler_event, ((1 << V_START_PORTAL_BIT_SET)));
        if (clk_mode == 0)
        {
            // prevent this unchanged parameter (false by default) to start tasks that should be stopped in clock mode
            vTaskResume(xTaskDaemon);
            vTaskResume(xTaskOnline);
            vTaskResume(xTaskOffline);
...
void timer_daemon(TimerHandle_t xTimer)
{
    ...
    if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS)
    {
        xEventGroupClearBits(handler_event, ((1 << SYSSTATE_IDLE_BIT_OFF) | (1 << SYSSTATE_IDLE_BIT_ON) | (1 << BTN_WAIT_MODE_BIT)));
...
{
    // called when timer expires
    xEventGroupSetBits(handler_event, (1 << SCD_CAL_DONE)); // set the calibration bit to 1
    if (debug_msk & (1 << debug_sensors))
    {
