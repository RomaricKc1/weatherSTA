...
void fcastLogic(const char *pcTaskName, const String serverPath_forecast,
    ...
    if (!(httpResponseCode > 0)) {
        if (debug_msk & (1 << debug_api)) {
            id_n_talk(pcTaskName,
                      "Forecast Error code: " + (String)httpResponseCode);
        }
        return;
    }
    ...
    if (error) {
        if (debug_msk & (1 << debug_api)) {
            id_n_talk(pcTaskName,
                      "Forecast Error code:deserializeJson() failed: " +
                          (String)error.c_str());
        }
        return;
    }
    ...
    if (debug_msk & (1 << debug_api)) {
        id_n_talk(pcTaskName,
                  "Sent fcast, f like: " + (String)data_f.feels_like);
    }
}
...
void nowLogic(const char *pcTaskName, const String serverPath_now,
    ...
    if (!(httpResponseCode > 0)) {
        if (debug_msk & (1 << debug_api)) {
            id_n_talk(pcTaskName,
                      "Now Error code: " + (String)httpResponseCode);
        }
        return;
    }
...
void handleMenuLogic(menu_t &menu_n, const int i, int &data_nexter,
    ...
    case 0: // exec aka cycle the current menu page variable (1x click)
        ...
        // pick the value for this exact index and menu page using preset
        // available, and inc the idx
    ...
    case 3: // exit menu before timeout (long press)
        // checks the different state, validate the bit, remove the current
        // timer, and create a new one really fast (like 2ms) that will expire
        // right away
...
void applyMenuSave(menu_t &menu_n, long &current_param, bool &clk_24_mode,
    ...
    case 2: // sleep param
        if (current_param != 0) {
            ...
            xQueueOverwrite(
                xIdle_T, static_cast<void *>(
                             &IDLE_TMS)); // local so gotta send it to the queue
        }
        break;
    ...
    case 4: // AM / PM mode
        ...
        xQueueOverwrite(x24h_Mode, static_cast<void *>(&clk_24_mode));
        break;
    case 5: // WLAN portal ondemand
        chg_wlan = current_param;
        xQueueOverwrite(xPortalMode, static_cast<void *>(&chg_wlan));
        break;
    case 0: // clock mode
        clk_mode = current_param;
        xQueueOverwrite(xClockMode, static_cast<void *>(&clk_mode));
...
void exitBeforeTimeout(const bool &clk_mode, bool &valid_clk, bool &ghost_timer,
    ...
    if (tmr_flag == true) {
        ...
        bool res_timer = createTimer(pdMS_TO_TICKS(2), releaser_daemon,
                                     "vSetup", "one-x-timer");
...
void printIndoorLCD(const dataOffline_t &my_data_offline,
    ...
    if (was_co2 == true) {
        ...
        if (ticktime_server >= (SENSORS_MS / LED_MS)) {
            ...
            rc = xQueuePeek(xInfluxDB_state,
                            static_cast<void *>(&db_server_state), 0);
        }

        if ((debug_msk & (1 << debug_dbserver)) && rc == pdPASS) {
            id_n_talk(pcTaskName,
                      "Recvd server state: " + (String)db_server_state);
        }
    ...
    if ((int)(round(my_data_offline.temp)) >= 0 &&
        ...
        lcd.setCursor(3, 0);
        lcd.print(" ");
    } else if ((int)(round(my_data_offline.temp)) < -9) { //-10, -11, ...
        lcd.setCursor(3, 0);
        lcd.write(byte(0));
    }
    ...
    } else if ((int)my_data_offline.humi <= 99) {
...
void printOutdoorLCD(const float outTemp) {
    ...
    if (xQueuePeek(xTempOut,
                   const_cast<void *>(static_cast<const void *>(&outTemp)),
                   0) == pdFAIL) {
    ...
    if (((int)(round(outTemp))) >= 0 and
        ...
        lcd.setCursor(15, 1);
        lcd.print(" ");
    } else if ((int)(round(outTemp)) < -9) { //-10, -11, ...
        lcd.setCursor(15, 1);
        lcd.write(byte(0));
    }
...
void driveRainLed(const String &rain, String *raining, const int size) {
    if (xQueuePeek(xQueueRain,
                   const_cast<void *>(static_cast<const void *>(&rain)),
                   0) == pdFAIL) {
        return;
    }
...
void onlineTaskPrint(const bool conf_active, dataOnline_t &my_data,
    ...
    (read_now)
        ? rc = xQueuePeek(xQueueOn_NOW, static_cast<void *>(&my_data), 0)
        : rc = xQueuePeek(xQueueOn_FCAST, static_cast<void *>(&my_data), 0);

    if (rc == pdPASS and
        (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS)) {
        clear_lcd();
        ...
        // others
        String text2 =
            "                Feels like: " + (String)my_data.feels_like +
            (String)((char)223) +
...
void offlineTaskPrint(const bool conf_active, dataOffline_t &my_data,
    ...
    rc = xQueuePeek(xQueueOffline, static_cast<void *>(&my_data), 0);
    if (rc == pdPASS and
        (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS)) {
        clear_lcd();
        ...
        (interrupted_ == true) ? validated_state = false
        ...
        bool interrupted = scroll_text(text2, LCD_SPD, 1);
        (interrupted == true) ? validated_state = false
                              : validated_state = true;
...
bool scdSetup() {
    ...
    if (error) {
        if (debug_msk & (1 << debug_sensors)) {
            ...
            id_n_talk(pcTaskName,
                      "Can't wakeup(), dead? lol: " + (String)errorMessage);
        }
    ...
    if (error) {
        if (debug_msk & (1 << debug_sensors)) {
            errorToString(error, errorMessage, sizeof errorMessage);
            id_n_talk(pcTaskName, "Can't stopPeriodicMeasurement(): " +
                                      (String)errorMessage);
        }
    ...
    if (error) {
        if (debug_msk & (1 << debug_sensors)) {
            errorToString(error, errorMessage, sizeof errorMessage);
            id_n_talk(pcTaskName, "Can't reinit(): " + (String)errorMessage);
        }
    ...
    scd_cal_one_x_timer =
        xTimerCreate("scd_calibration_timer", pdMS_TO_TICKS(1000), pdFALSE,
                     static_cast<void *>(0), scd_cal_daemon);
...
void server_init_stuff(on_Data_t &on_now, on_Data_t &on_fcast,
    ...
    server.on(
        "/jquery-3.6.0.min.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    server.on(
        "/d_slp_duration", HTTP_POST, [&](AsyncWebServerRequest *request) {
            if (request->hasParam("value_d_slp_duration", true)) {
                String msg =
                    request->getParam("value_d_slp_duration", true)->value();
    ...
    server.on("/outhumidity_now", HTTP_GET,
              [&](AsyncWebServerRequest *request) {
                  request->send(200, "text/plain", (String)on_now.humidity);
              });
    server.on("/outpressure_now", HTTP_GET,
              [&](AsyncWebServerRequest *request) {
    ...
    server.on("/outfeelslike_now", HTTP_GET,
              [&](AsyncWebServerRequest *request) {
    ...
    server.on("/outwindspeed_now", HTTP_GET,
              [&](AsyncWebServerRequest *request) {
    ...
    server.on("/outclouds_fcast", HTTP_GET,
              [&](AsyncWebServerRequest *request) {
    ...
    server.on(
        "/outdescription_fcast", HTTP_GET, [&](AsyncWebServerRequest *request) {
...
void getServer_State(bool &state, const char *pcTaskName) {
    ...
    InfluxDBClient db_client =
        InfluxDBClient(user_influx_url, user_influx_org, user_influx_bucket,
                       user_influx_token);
    // check server state
...
bool writePointToServer(Point &sensor, dataOffline_t data,
    ...
    if (debug_msk & (1 << debug_dbserver)) {
        id_n_talk(pcTaskName, "Trying to write: " + sensor.toLineProtocol());
    }
    ...
    if (db_client.writePoint(sensor) == false) {
        if (debug_msk & (1 << debug_dbserver)) {
            id_n_talk(pcTaskName, "InfluxDB write failed: " +
                                      db_client.getLastErrorMessage());
        }
        failed_try += 1;
        return false;
...
void updateServerValues(TickType_t xTick_offline, dataOffline_t &my_data_off,
    ...
    if ((xTaskGetTickCount() - xTick_offline) > 10 * 1000) {
        // 10s for local temp
        xQueuePeek(xQueueOffline, static_cast<void *>(&my_data_off), 0);
    ...
    if ((xTaskGetTickCount() - xTick_online) > 30 * 1000) {
        ...
        xQueuePeek(xQueueOn_NOW, static_cast<void *>(&my_data), 0);
        update_onl_values(on_now, my_data);

        xQueuePeek(xQueueOn_FCAST, static_cast<void *>(&my_data), 0);
...
void readSysConfig() {
    ...
    if (!(SPIFFS.exists(CONFIG_FILE))) {
        ...
        if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS) {
            clear_lcd();
            scroll_text("        API KEY and Location required. Opening config "
                        "portal...",
                        LCD_SPD, 0);
            scroll_text("        will timeout after 6mins, starting now.",
                        LCD_SPD, 1);
            xSemaphoreGive(xLCDAccess);
        }
        ...
        if (!res) {
            if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS) {
                clear_lcd();
                scroll_text("        failed to connect or timed out", LCD_SPD,
                            0);
                xSemaphoreGive(xLCDAccess);
            }
            // then what next? restart? yes, after the timeout so it's long, but
            // user can hit restart right away
        } else {
            clear_lcd();
...
bool createTimer(const long duration, TimerCallbackFunction_t(call_func_ptr),
    ...
    one_x_timer_releaser =
        xTimerCreate(timerName, pdMS_TO_TICKS(duration), pdFALSE,
                     static_cast<void *>(0), call_func_ptr);
...
void resolveInputs(menu_t &menu_n, const int i, int &data_nexter,
    ...
    vTaskSuspend(xTaskOnline); // suspend those who have access to the lcd
    ...
    handleMenuLogic(menu_n, i, data_nexter, current_param, clk_24_mode,
...
void clockOrStaMode() {
    ...
    case 1: {
        ...
        // clock mode
        // cannot just suspend them, lol let them release the mutex before
        // proceding
        broadcastConfMode();
...
void wifiCoIssues(TickType_t &xTick_wlan_state, const bool valid_clk,
    ...
    if (((xTaskGetTickCount() - xTick_wlan_state) >= 1 * 60 * 1000) and
        ...
        if (valid_clk == true) {
            ...
            if (debug_msk & (1 << debug_setup)) {
                id_n_talk("vTaskSetup",
                          "took ctrl of led, starting flickering");
            }
            ...
            if ((cnt > (10 / (int)(LED_MS * 2 / 1000))) &&
                reboot_reconfd == false) {
    ...
    if (go_reboot == true) {
        ...
        // these will assure deep sleep and wake up to clock mode again after 10
        // s
        reboot_reconfd = true;
...
void pirLogic(bool &pir_state, bool &last_pir, bl_mgmt_t &that_mgmt,
    ...
    {
        if (that_mgmt.turned_on == true and
            xSemaphoreTake(xI2C, 200) == pdPASS) {
            if (that_mgmt.extended_bl == false or
...
void idleSTAMode(bool conf_active, bool &hard_resume, int &rep,
    ...
    if (conf_active == true) {
        ...
        // xSemaphoreGive(xLCDAccess); // give it to config thing, and it will
        // block you lol
        return;
    }
    ...
    if (hard_resume == true) {
        ...
        rep += 1; // and yea, only if you did yo thing nicely you can count it,
                  // not while config was waiting
...
void saveParamCallback() {
    ...
    if (!config) {
        if (xSemaphoreTake(xLCDAccess, portMAX_DELAY) == pdPASS) {
            clear_lcd();
            scroll_text(
                "                Error opening config file. Rebooting ...",
                LCD_SPD, 0);
...
void releaser_daemon(TimerHandle_t xTimer) {
    ...
    xQueueReceive(xClockMode, static_cast<void *>(&clk_mode), 0);
    xQueueReceive(xPortalMode, static_cast<void *>(&chg_config), 0);
    ...
    if (chg_config == true) {
        xEventGroupSetBits(
            handler_event,
            ((1
              << V_START_PORTAL_BIT_SET))); // validate the portal mode now
